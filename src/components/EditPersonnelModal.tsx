  ...
  const onSubmit = async (values: EditPersonnelFormValues) => {
    ...
    try {
      ...
      const { data: edgeFunctionData, error: edgeFunctionError } = await supabase.functions.invoke('update-user-metadata', {
        body: { userId: personnel.id, firstName: values.first_name, lastName: values.last_name },
      });
      ...
      if (error instanceof Error) toast.error(`Gagal memperbarui profil: ${error.message}`);
      else toast.error("Gagal memperbarui profil: Terjadi kesalahan tidak diketahui");
    }
  };
